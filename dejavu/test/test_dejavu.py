...
class UnitTests(unittest.TestCase):
    ...
    def test_sandbox_cache(self):
        # Make sure the _sandbox_ cache is being used, not the ObjectCache's.
        ...
        bat3 = store.new_sandbox().unit(Animal)
    ...
    def test_json(self):
        try:
            from dejavu.json import Converter, Encoder, Decoder, unit_to_dict, dict_to_unit
        except ImportError:
        ...
        zoo_json = json.dumps(unit_to_dict(zoo))
        
        self.assert_("1916-10-02" in zoo_json)
        self.assert_("__dejavu.class__" in zoo_json)
        
        zoo_json = zoo_json.replace("San Diego Zoo", "The San Diego Zoo")
        zoo2 = dict_to_unit(json.loads(zoo_json))
