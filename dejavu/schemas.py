...
class Schema(object):
    ...
    class MySchema(dejavu.Schema):
        ...
        def upgrade_to_1(self):
            ...
            self.store.drop_property(Employee, "ZipCode")
            self.store.add_property(Employee, "StartDate")
    
    Then, in your deployment scripts, you can choose how much you want to
    automate control of the schema.
    
        schema = MySchema(mystore)
    ...
    def deployed_unit(self, default=None):
        ...
        if not self.store.has_storage(DeployedVersion):
            self.store.map([DeployedVersion], conflicts='repair')
    ...
    def _set_dep(self, newvalue):
        ...
        self.store.save(depunit, forceSave=True)
    ...
    def assert_storage(self):
        ...
        """
        self.store.map_all(conflicts='repair')
